...
function emptyBin() {
  ...
  showConfirmModal(
    ...
    'Delete All', '#ff4444',
    () => { _emptyBinConfirmed(count); }
  );
}

async function _emptyBinConfirmed(count) {
  ...
  try {
    ...
    else showToast(`🗑️ Bin emptied — ${count} item(s) permanently deleted`);
  } catch (e) {
...
async function permanentDelete(id) {
  ...
  const hasRemote = !!(p.pcFileId || p.thId);
  try {
    if (hasRemote) showToast('🗑️ Deleting…');
    const r = await _deletePhotosFromCloud([p]);
    if (r.queued > 0) showToast('🗑️ Deleted here — cloud copy will be removed when Pcloud is reachable');
    else showToast('🗑️ Permanently deleted');
  } catch (e) {
    showToast('⚠️ Deleted here — cloud cleanup will retry: ' + (e.message || ''));
    console.warn('permanentDelete cleanup error:', e);
  }
}
          ...
          if (Math.abs(dragDX) > SWIPE_COMMIT) {
            ...
            setTimeout(function() {
              ...
              if (idx >= 0) {
                ...
                _deletePhotosFromCloud([p]).catch(function(){});   // thumbnail, caches, blob + Pcloud files
                _saveTrashed();
                save();
...
function lbAction_delete() {
  ...
  // Opened from the Bin → this is the permanent delete (never re-add to the Bin)
  if (typeof trashedPhotos !== 'undefined' && trashedPhotos.some(x => x.id === p.id)) {
    showConfirmModal(
      'Delete permanently?',
      '"' + (p.name || 'this file') + '" will be deleted forever. This cannot be undone.',
      'Delete', '#ff4444',
      () => {
        const ti = trashedPhotos.findIndex(x => x.id === p.id);
        if (ti !== -1) trashedPhotos.splice(ti, 1);
        _saveTrashed();
        sliderList.splice(sIndex, 1);
        _deletePhotosFromCloud([p]).catch(() => {});
        try { _renderBinItems(); } catch (e) {}
        _updateBinBadge();
