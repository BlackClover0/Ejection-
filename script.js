...
function _upSetHero(txt) {
  ...
  const u = document.createElement('span'); u.style.fontSize = '17px'; u.textContent = m[3]; e.appendChild(u);
}
/* stage: 0 idle, 1 uploading, 2 saving library, 3 done */
function _upStage(n) {
  const w = (id, v) => { const e = document.getElementById(id); if (e) e.style.width = v; };
  const c = (id, on) => { const e = document.getElementById(id); if (e) e.style.color = on ? 'var(--gold)' : ''; };
  w('upSeg1', n >= 1 ? '100%' : '0%');
  w('upSeg3', n >= 3 ? '100%' : n === 2 ? '50%' : '0%');
  c('upLbl1', n >= 1); c('upLbl2', n >= 1); c('upLbl3', n >= 2);
}
...
async function uploadRefreshStats() {
  ...
  try {
    ...
    set('upStatVaultN', String(s.cloudN + s.thN + libFiles));
