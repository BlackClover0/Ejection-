...
async function unlockVault() {
  ...
  _flushPendingUploads(); // safety net — cheap no-op if nothing's queued
}
...
/* Runs automatically every time the vault is unlocked (fresh unlock, first-
   ...
   reappear on their own — no manual "Retrieve from pCloud" tap needed.
   Deliberately quiet: unlike tgRetrieveAll() (the manual button), this
   should never greet the person with a scary error toast just because
   they haven't uploaded anything yet or a request briefly failed. */
async function _autoRestoreFromCloud() {
  ...
  // rather than blocking unlock; the manual "Retrieve from pCloud" button
  // (tgRetrieveAll) still waits its turn if the person asks for it directly.
  ...
  try {
    ...
    if (!cloudPhotos.length) return; // nothing uploaded yet — stay quiet
    ...
    if (added > 0) {
      ...
      showToast(`☁️ ${added} photo${added !== 1 ? 's' : ''} restored from pCloud`);
      _setActivityStatus(`${added} file${added !== 1 ? 's' : ''} retrieved from cloud`, 'fa-cloud-arrow-down', 'cloud');
    }
  } catch(e) {
    // A missing manifest (nothing pushed yet) or a transient network hiccup
    // shouldn't interrupt opening the app. The manual "Retrieve from
    // pCloud" button in ☁️ Cloud Settings still surfaces real errors
    // loudly if the person explicitly asks for a retry.
...
/* Activity tracker — two bars on the homepage, side by side in one row.
   kind 'local' updates the left bar (images uploaded & encrypted on-device);
   kind 'cloud' updates the right bar (files uploaded to / retrieved from
   cloud). iconClass is a Font Awesome class (defaults to a check icon).
   Fails silently if the card isn't in the DOM yet (e.g. called before
   first render). */
function _setActivityStatus(text, iconClass, kind) {
  const suffix = kind === 'cloud' ? 'Cloud' : 'Local';
  const textEl = document.getElementById('activity' + suffix + 'Text');
  const iconEl = document.getElementById('activity' + suffix + 'Icon');
  const timeEl = document.getElementById('activity' + suffix + 'Time');
  if (!textEl || !iconEl || !timeEl) return;
  textEl.textContent = text;
  iconEl.innerHTML = `<i class="fas ${iconClass || 'fa-circle-check'}"></i>`;
...
(function () {
  ...
  function syncStats() {
    ...
    var countValEl = document.getElementById('dcCountValue');

    if (headerCount) {
      var countMatch = headerCount.textContent.match(/[\d,]+/);
      window._itemsCount = countMatch ? countMatch[0] : '0';
      if (countValEl) countValEl.textContent = window._itemsCount;
    }
...
function _resetPg2Cards() {
  ...
  if (sc) sc.innerHTML = `
    <div class="dash-card accent-5" style="cursor:pointer;" onclick="openAllFoldersSheet()">
      <div class="dash-icon"><i class="fas fa-folder-open"></i></div>
      <div><div id="ccStarredBadge" class="dash-value">${starredN}</div><div class="dash-label">folders</div></div>
      <i class="fas fa-chevron-right dash-chevron"></i>
    </div>
    <div class="dash-card accent-4" style="cursor:pointer;" onclick="_collectGo('favourites')">
      <div class="dash-icon"><svg viewBox="0 0 100 108" fill="none" xmlns="http://www.w3.org/2000/svg" style="width:16px;height:16px;"><path d="M65.7 10.8 L65.8 43.4 L96.0 55.6 L65.0 65.7 L62.7 98.2 L43.5 71.9 L11.9 79.7 L31.0 53.3 L13.8 25.7 L44.8 35.7 Z" stroke="currentColor" stroke-width="9" stroke-linejoin="round"/></svg></div>
      <div><div id="ccFavsBadge" class="dash-value">${favsN}</div><div class="dash-label">picked</div></div>
      <i class="fas fa-chevron-right dash-chevron"></i>
    </div>`;

  const mc = document.getElementById('pg2MediaCard');
  if (mc) mc.innerHTML = `
    <div class="pg2-row hp-row pg2-media-row" onclick="_collectGo('images')">
      <div class="hp-icon-badge accent-2"><i class="far fa-image"></i></div>
      <div class="hp-row-label">Images</div>
      <span id="ccImagesBadge" class="hp-row-badge">${imgsN}</span>
      <i class="fas fa-chevron-right hp-chevron"></i>
    </div>
    <div class="pg2-row hp-row pg2-media-row" onclick="_collectGo('videos')">
      <div class="hp-icon-badge accent-1"><i class="fas fa-video"></i></div>
      <div class="hp-row-label">Videos</div>
      <span id="ccVideosBadge" class="hp-row-badge">${vidsN}</span>
      <i class="fas fa-chevron-right hp-chevron"></i>
    </div>
    <div class="pg2-row hp-row pg2-media-row" onclick="_collectGo('gifs')">
      <div class="hp-icon-badge accent-4"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" style="width:16px;height:16px;"><path d="M4 3.5 C4 2.5 5 2.2 5.8 2.7 L19 10.2 C19.8 10.7 19.8 11.8 19 12.3 L5.8 19.8 C5 20.3 4 19.9 4 18.9 Z" fill="currentColor"/><path d="M3 14.5 Q10 12.5 20 13.5" fill="none" stroke="#0a0a0c" stroke-width="2" stroke-linecap="round"/></svg></div>
      <div class="hp-row-label">GIFs</div>
      <span id="ccGifsBadge" class="hp-row-badge">${gifsN}</span>
      <i class="fas fa-chevron-right hp-chevron"></i>
    </div>
    <div class="pg2-row hp-row pg2-media-row" onclick="_collectGo('webps')">
      <div class="hp-icon-badge accent-3"><img src="data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAxMDAgMTAwIj48cG9seWdvbiBwb2ludHM9IjUsMyA5Nyw0NyA1LDQ3IiBmaWxsPSIjYzhhZDdhIi8+PHBvbHlnb24gcG9pbnRzPSI1LDk3IDk3LDUzIDUsNTMiIGZpbGw9IiNjOGFkN2EiLz48L3N2Zz4=" style="width:15px;height:15px;object-fit:contain;"></div>
      <div class="hp-row-label">WebP</div>
      <span id="ccWebpsBadge" class="hp-row-badge">${webpsN}</span>
      <i class="fas fa-chevron-right hp-chevron"></i>
    </div>`;

  const mo = document.getElementById('pg2MoreCard');
  if (mo) mo.innerHTML = `
    <div id="ccBinTag" class="pg2-row hp-row" onclick="openBinModal()">
      <div class="hp-icon-badge accent-6"><i class="fas fa-trash"></i></div>
      <div class="hp-row-label">Bin</div>
      <span id="ccBinBadge" class="hp-row-badge" style="display:${binShow?'inline':'none'};">${binText}</span>
      <i class="fas fa-chevron-right hp-chevron"></i>
    </div>`;
}
...
const PCLOUD_APP_KEY = ''; // TODO: paste App Key from the pCloud App Console
...
/* ── Cloud direction lock ──
   Uploading TO pCloud (tgPushAll / _flushPendingUploads / syncToCloud) and
   pulling FROM pCloud (tgRetrieveAll / _autoRestoreFromCloud / the polling
...
async function pcConnect() {
  if (!PCLOUD_APP_KEY) { showToast('⚠️ Set PCLOUD_APP_KEY first (App Console → App key).'); return; }
  ...
  sdk.oauth.initOauthPollToken({
    ...
    receiveToken: function(access_token, locationid) {
      ...
      _tgRefreshStatusUI();
      updateCloudUI();
      // Anything added before pCloud was ever connected has been sitting
      // in the pending-uploads queue — start delivering it right away.
...
async function tgPushAll() {
  ...
  if (localPhotos.length === 0) {
    ...
    if (status) status.textContent = 'All photos are already in pCloud.';
  ...
  if (title) title.textContent = `Uploading ${localPhotos.length} file(s) to pCloud…`;
  ...
  for (const p of localPhotos) {
    try {
      ...
      // Upload to pCloud — filename includes .enc so it's clear it's encrypted
      const safeName = (p.name || 'file').replace(/[^a-zA-Z0-9._-]/g, '_');
      const pcFileId = await _pcUploadFile(encBlob, 'hope_' + p.id + '_' + safeName + '.enc');
      ...
      // Update photo record — now it lives in pCloud
      p.storage  = 'cloud';
      p.pcFileId = pcFileId;
      delete p.encData;
      delete p.encId;
  ...
  // Save updated manifest to pCloud
  try {
...
let _tgRetrieveBusy = false;

async function tgRetrieveAll() {
  if (!isPcloudEnabled()) { showToast('⚠️ Connect pCloud first!'); openCloudModal(); return; }
  if (_tgRetrieveBusy) { showToast('⏳ Already retrieving from pCloud…'); return; }
  _tgRetrieveBusy = true;
  await _acquireCloudDirection('retrieve'); // waits out any in-flight upload
  ...
  if (bar) bar.style.width = '0%';
  if (icon) { icon.className = 'fas fa-rotate'; icon.style.cssText = 'animation:spin 1s linear infinite;'; }
  if (title) title.textContent = 'Connecting to pCloud…';
  ...
  try {
    ...
    if (cloudPhotos.length === 0) {
      if (title) title.textContent = '📭 No files in pCloud yet';
      if (status) status.textContent = 'Upload some files first using "Upload All to pCloud".';
      showToast('📭 No files found in pCloud');
      return;
    }

    if (title) title.textContent = `Found ${cloudPhotos.length} file(s) — fetching…`;

    // Merge cloud photos into local photos array (in-memory only, not saved to localStorage)
    ...
    if (icon) { icon.className = 'fas fa-circle-check'; icon.style.animation = 'none'; }
    if (title) title.textContent = `✅ ${cloudPhotos.length} file(s) ready to view!`;
    if (status) status.textContent = 'Tap any photo to decrypt and view it. Nothing is saved to your device.';

    showToast(`✅ ${cloudPhotos.length} file(s) ready to view! Nothing saved to device.`);
    _setActivityStatus(`${cloudPhotos.length} file(s) retrieved from cloud`, 'fa-cloud-arrow-down', 'cloud');
    ...
    if (status) status.textContent = e.message || 'Check your pCloud connection.';
    showToast('❌ ' + (e.message || 'Retrieve failed. Reconnect pCloud in ☁️ Cloud Settings.'));
  } finally {
    _tgRetrieveBusy = false;
    _releaseCloudDirection('retrieve');
  }
}
...
function load() {
  ...
  // hadn't finished uploading to pCloud yet (app closed mid-upload, no
  // signal, etc.) is reconstructed from the pending-uploads queue below and
  // handed straight back to the background uploader — otherwise "surely
  // gets uploaded" wouldn't survive a closed app. Its thumbnail still can't
  ...
  // Kick off delivery immediately — doesn't need the PIN, since the blobs
  // are already encrypted ciphertext sitting in IndexedDB. Safe to call
  // even if pCloud isn't connected or the queue is empty (it just no-ops).
  _flushPendingUploads();
}
...
/* ── Pending-upload queue ──
   ...
   to IndexedDB but not yet confirmed uploaded to pCloud. This is what makes
   "surely gets uploaded" survive the app being closed mid-upload: the
   actual ciphertext lives in IndexedDB (untouched by load() above), and
   this queue carries just enough plain metadata (name/folder/size/etc,
   the same fields already stored in plaintext in the pCloud manifest) to
   rebuild the photo record and retry the upload next time the app opens. */
...
/* Re-adds any queued-but-unconfirmed items to `photos` on startup so they
   ...
   pCloud by _flushPendingUploads() below, which matches queue entries
   against `photos` by id to update storage/pcFileId in place on success. */
...
let _uploadFlushInProgress = false;
let _uploadFlushRetryTimer = null;

/* Uploads every queued item to pCloud, retrying transient failures with
   backoff, and reschedules itself automatically until the queue is empty —
   this is what "surely gets uploaded" means in practice: a flaky
   connection delays delivery, it doesn't lose the file. Safe to call as
   often as you like; concurrent calls collapse into one run. */
async function _flushPendingUploads() {
  if (_uploadFlushInProgress) return;
  if (!isPcloudEnabled()) return;
  const queue = _getPendingQueue();
  if (!queue.length) return;

  _uploadFlushInProgress = true;
  await _acquireCloudDirection('upload'); // waits out any in-flight retrieve
  let pushedAny = false;

  for (let i = 0; i < queue.length; i++) {
    const entry = queue[i];
    // Skip anything the user deleted from `photos` while it was still queued
    // (e.g. trashed before it ever made it to the cloud).
    const live = photos.find(p => p.id === entry.id);
    if (!live) { _removeFromPendingQueue(entry.id); continue; }
    if (live.storage === 'cloud' && live.pcFileId) { _removeFromPendingQueue(entry.id); continue; }

    _setActivityStatus(`Uploading to cloud… (${i + 1}/${queue.length})`, 'fa-cloud-arrow-up', 'cloud');

    let encBlob;
    try { encBlob = await idbGet(entry.id); }
    catch(e) { encBlob = null; }
    if (!encBlob) { _removeFromPendingQueue(entry.id); continue; } // blob genuinely gone — nothing to retry

    // Up to 3 attempts per item per flush pass, short backoff between them —
    // most transient failures (brief network drop) clear within a few seconds.
    let uploaded = false, lastErr = null;
    for (let attempt = 1; attempt <= 3 && !uploaded; attempt++) {
      try {
        const safeName = (entry.name || 'file').replace(/[^a-zA-Z0-9._-]/g, '_');
        const pcFileId = await _pcUploadFile(encBlob, 'hope_' + entry.id + '_' + safeName + '.enc');
        live.storage  = 'cloud';
        live.pcFileId = pcFileId;
        delete live.encData;
        delete live.encId;
        idbDelete(entry.id).catch(() => {});
        _removeFromPendingQueue(entry.id);
        uploaded = true;
        pushedAny = true;
      } catch(e) {
        lastErr = e;
    ...
    if (!uploaded) console.warn('pCloud upload still pending for', entry.name, '—', lastErr && lastErr.message);
  }

  _uploadFlushInProgress = false;

  if (pushedAny) {
    try { await _tgSaveManifest(); } catch(e) { console.warn('Manifest sync after flush failed:', e); }
    if (typeof render === 'function') render();
    if (typeof _updateStoragePill === 'function') _updateStoragePill();
    _setActivityStatus('Synced to cloud', 'fa-cloud-arrow-up', 'cloud');
  }

  _releaseCloudDirection('upload');

  // Anything left over (still offline, quota hit, etc.) gets another pass
  // shortly — keeps retrying quietly rather than giving up on the file.
  const remaining = _getPendingQueue();
  if (_uploadFlushRetryTimer) clearTimeout(_uploadFlushRetryTimer);
  if (remaining.length > 0) {
    _uploadFlushRetryTimer = setTimeout(() => _flushPendingUploads(), 20000);
  }
}

// Resume the moment connectivity comes back, instead of waiting for the timer.
window.addEventListener('online', () => _flushPendingUploads());
...
async function fetchCloudBlob(p) {
...
function restoreFromBin(id) {
  ...
  photos.push(p);
  if (p.storage === 'idb' && p.encId) { _addToPendingQueue(p); _flushPendingUploads(); }
  _saveTrashed();
  save(); render();
...
function switchToPage(n) {
  ...
  if (track) {
    ...
    clearTimeout(track._wcTimer);
    track._wcTimer = setTimeout(() => { track.style.willChange = 'auto'; }, 480);
  }
  const tp = document.getElementById('tabPhotos');
  const ta = document.getElementById('tabAlbums');
...
/* ── Core upload pipeline — wired to the single global #fileInput used by
   every "Add Files" trigger in the app (homepage add button, folder
   detail pages, subfolders, and each media-type collection page).
   Encrypts each selected file with the current session PIN and saves the
   ciphertext to IndexedDB immediately (fast — the gallery updates right
   away), then hands the whole batch to _flushPendingUploads() in the
   background to actually reach pCloud. Nothing is awaited here past the
   local save, so adding files feels instant regardless of network speed;
   delivery to the cloud is guaranteed separately (see _flushPendingUploads
   and the pending-uploads queue above) — including resuming automatically
   if the app is closed before a batch finishes uploading. ── */

/* ── Local encryption queue ──
   Encryption itself must stay strictly serial — only one file-selection
   batch is ever being encrypted + written to IndexedDB at a time. Picking
   new files while a batch is mid-encryption is still instant (it just adds
   another batch to the queue); the queued batch's actual encrypt/save work
   waits its turn instead of running alongside the current one. Once a
   batch finishes encrypting, its cloud push (_flushPendingUploads) runs in
   the background — concurrently with the *next* queued batch encrypting. */
let _encryptQueue    = [];
...
async function _encryptAndSaveBatch({ files, folderId, pendingType }) {
  ...
  _setActivityStatus(`Uploading ${files.length} file${files.length !== 1 ? 's' : ''} to app…`, 'fa-arrow-up-from-bracket', 'local');
  ...
  for (const file of files) {
    try {
      ...
      added++;
      // Refresh as soon as THIS file lands — this is what makes files show
      // up while a 20-30 file batch is still in progress, instead of only
      // after the very last file finishes encrypting.
  ...
  // covers the all-failed case too (empty toast/status still need updating).
  if (typeof save === 'function') save();
  if (typeof render === 'function') render();
  if (typeof renderFolders === 'function') renderFolders();
  if (typeof _updateStoragePill === 'function') _updateStoragePill();
  ...
  if (added > 0) {
    const cloudNote = isPcloudEnabled() ? ' — uploading to pCloud…' : '';
    showToast(`Added ${added} file${added !== 1 ? 's' : ''}${cloudNote}${failed ? `, ${failed} failed` : ''}`, 'success');
    _setActivityStatus(`Encrypted & saved ${added} file${added !== 1 ? 's' : ''} locally`, 'fa-lock', 'local');
  } else if (failed > 0) {
    showToast(`❌ Failed to add ${failed} file${failed !== 1 ? 's' : ''}`);
  }

  // Fire-and-forget: the batch push to pCloud happens in the background so
  // this batch's encryption doesn't wait on the network, and it runs
  // alongside the *next* queued batch's encryption too.
  // _flushPendingUploads() retries on its own until every queued item is
  // confirmed delivered, including across app restarts (see the
  // pending-uploads queue above).
  _flushPendingUploads();
}
