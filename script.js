...
async function _tgSaveManifest() {
  const manifest = {
    ...
    photos: photos.map(p => ({
      ...
      thSize: p.thSize || undefined, pvSize: p.pvSize || undefined,
    })),
    folders
  };
...
async function _vcPutDerivs(p, rec) {
  ...
  if (!p.pvId) { p.pvId = await _vcUploadRetry(rec.p, 'hope_' + p.id + '_pv.enc'); p.pvSize = rec.p.size; }
}
...
async function _vcOrigTrim() {
  ...
  } catch (e) {} finally { _vcTrimming = false; _vcScheduleExtra(); }
}
...
async function _vcLoadIndexIntoPhotos() {
  ...
  } finally { _vcReady = true; _vcScheduleExtra(); }
}
document.addEventListener('visibilitychange', () => {
...
function _vcMergeManifest(manifest) {
  ...
  for (const cp of cloudPhotos) {
    if (!existingIds.has(cp.id)) { photos.push(cp); existingIds.add(cp.id); added++; }
      ...
      if (lp && lp.storage === 'cloud') { if (!lp.thId && cp.thId) lp.thId = cp.thId; if (!lp.pvId && cp.pvId) lp.pvId = cp.pvId; if (!lp.thSize && cp.thSize) lp.thSize = cp.thSize; if (!lp.pvSize && cp.pvSize) lp.pvSize = cp.pvSize; }
    }
  }
...
async function _vcLocalState() {
  const hasT = new Set(), hasP = new Set(), szT = new Map(), szP = new Map();
  ...
  await new Promise((res, rej) => {
    ...
    rq.onsuccess = () => {
      ...
      if (c.value && c.value.p) { hasP.add(c.key); szP.set(c.key, c.value.p.size || 0); }
      c.continue();
    };
    rq.onerror = () => rej(rq.error);
  });
  return { hasT, hasP, szT, szP };
}

/* storage numbers for the Upload and Retrieve pages */
async function _vcStorageStats() {
  const { hasT, hasP, szT, szP } = await _vcLocalState();
  let cache = 0;
  ...
  const s = {
    ...
    thN: 0, th: 0, thUnk: 0, pvN: 0, pv: 0, pvUnk: 0,          // in Pcloud
    devTn: 0, devT: 0, devPn: 0, devP: 0, cache,                 // on this device
  ...
  for (const p of cloud) {
    ...
    if (!p.pvSize && szP.has(p.id) && p.pvId) p.pvSize = szP.get(p.id);
    if (p.thId) { s.thN++; if (p.thSize) s.th += p.thSize; else s.thUnk++; } else s.noThumb++;
    if (p.pvId) { s.pvN++; if (p.pvSize) s.pv += p.pvSize; else s.pvUnk++; }
    if (hasT.has(p.id)) { s.devTn++; s.devT += szT.get(p.id) || 0; }
    if (hasP.has(p.id)) { s.devPn++; s.devP += szP.get(p.id) || 0; }
    if (p.thId && !hasT.has(p.id)) { s.getN++; if (p.thSize) s.get += p.thSize; else s.getUnk++; }
    if (p.pvId && !hasP.has(p.id)) { s.getN++; if (p.pvSize) s.get += p.pvSize; else s.getUnk++; }
  }
  s.szT = szT; s.szP = szP;
  return s;
}
...
async function retrieveRefreshStats() {
  ...
  try {
    ...
    set('rtStatCloud', (s.cloudN + s.thN + s.pvN + lib) + ' files \u00B7 ' + unk + _upFmtBytes(s.main + s.th + s.pv + libBytes));
...
async function uploadRefreshStats() {
  ...
  try {
    ...
    // ── waiting files, and what they will add to Pcloud (main + thumbnail + preview + library) ──
    const waiting = photos.filter(_isWaitingForUpload);
    const avgT = s.thN - s.thUnk > 0 ? s.th / (s.thN - s.thUnk) : 30 * 1024;
    const avgP = s.pvN - s.pvUnk > 0 ? s.pv / (s.pvN - s.pvUnk) : 150 * 1024;
    const avgL = s.cloudN ? cloudLib / s.cloudN : 350;
    let estT = 0, estP = 0, guessed = false;
    for (const p of waiting) {
      if (s.szT.has(p.id)) estT += s.szT.get(p.id); else { estT += avgT; guessed = true; }
      if (s.szP.has(p.id)) estP += s.szP.get(p.id); else { estP += avgP; guessed = true; }
    }
    ...
    set('upEstPrev', ap + F(estP));
    set('upEstLib', '\u2248 ' + F(estL));
    set('upEstTotal', '\u2248 ' + F(s.waiting + estT + estP + estL));

    // ── what is in Pcloud now ──
    const unk = (s.thUnk || s.pvUnk) ? '\u2265 ' : '';
    const libFiles = s.cloudN ? 1 : 0;
    set('upStatVault', (s.cloudN + s.thN + s.pvN + libFiles) + ' files \u00B7 ' + unk + F(s.main + s.th + s.pv + cloudLib));
...
function _vcTotals(s) {
  ...
  return {
    lib,
    device: lib + s.devT + s.devP + s.waiting + s.cache,      // held on this phone
    cloud: s.main + s.th + s.pv + (s.cloudN ? lib : 0),       // held in Pcloud
    cloudApprox: !!(s.thUnk || s.pvUnk),
  };
}
...
async function storageRefreshStats() {
  ...
  try {
    ...
    set('stLocPrev', s.devPn + ' \u00B7 ' + F(s.devP));
    ...
    set('stCloudPrev', s.pvN + ' \u00B7 ' + _stLabel(s.pv, s.pvUnk));
...
/* ════════════════════════════════════════════════════════════════════════
     Sidebar storage card: two separate numbers
        On device = library + thumbnails + previews + files waiting + recently-opened cache
        In cloud  = main files + thumbnails + previews + library (all in Pcloud)
