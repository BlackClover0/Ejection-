...
function openBinModal() {
  ...
  window._binSettleAt = Date.now() + 850;   // thumbnails start loading after the 800ms open animation
  _renderBinItems();
...
function _renderBinItems() {
  ...
  // Start thumbnails after the open animation, 3 at a time, and drop the queue if the grid is re-rendered
  setTimeout(() => {
    let next = 0;
    const worker = async () => {
...
async function openLightbox(startIndex, list) {
  ...
  window._lbSettleAt = Date.now() + 850;   // full-quality swap waits for the 800ms open animation
...
async function _vcFillSlide(idx, autoplay, wantFull) {
  ...
  const upgrade = (async () => {
    try {
      ...
      // Don't swap in the full file while the open animation is still running
      if (showedQuick) {
