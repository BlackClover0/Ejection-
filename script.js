...
async function _handleFileInputChange(e) {
  ...
  if (!sessionPin) {
    showToast('⚠️ Unlock the vault first.');
    return;
  }
  ...
  // adding files stays instant. _flushPendingUploads() retries on its own
  // until every queued item is confirmed delivered, including across app
  // restarts (see the pending-uploads queue above).
  _flushPendingUploads();
}
