...
/* ════════════════════════════════════════════════════════════════════════
     SECTION 9 — STORAGE PAGE  (opened from the storage card at the top of the sidebar)
     Shows what THIS PHONE holds for the vault.
     ════════════════════════════════════════════════════════════════════════ */
function openStoragePage(section) {
  const m = document.getElementById('storagePage');
  if (!m) return;
  m.classList.add('open');
  storageRefreshStats();
  // the device card lands on the top; the cloud card scrolls straight to the cloud section
  const body = m.querySelector('.fs-panel-body');
  if (body) body.scrollTop = 0;
  if (section === 'cloud' && body) {
    setTimeout(() => {
      const t = document.getElementById('stSecCloud');
      if (t) body.scrollTop = t.getBoundingClientRect().top - body.getBoundingClientRect().top + body.scrollTop - 8;
    }, 60);
  }
}
...
async function storageRefreshStats() {
  const set = (id, v) => { const e = document.getElementById(id); if (e) e.textContent = v; };
  const F = _upFmtBytes;
  try {
    const s = await _vcStorageStats();
...
/* ════════════════════════════════════════════════════════════════════════
     ...
     Tapping the card opens the Storage page with the full detail.
     ════════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════════
     SECTION 10 — CLEAR BUTTONS ON THE STORAGE PAGE (this device only, never the cloud)
