...
async function _tgPushAllCore() {
  ...
  if (sent > 0) { try { localStorage.setItem('pv_last_upload', String(Date.now())); } catch (e) {} }
  _saveLocalOnly();
  render();
...
async function retrieveRefreshStats() {
  ...
  if (note) note.style.display = isPcloudEnabled() ? 'none' : 'flex';
  try {
    ...
    const unk = s.thUnk ? '\u2265 ' : '';
    set('rtStatThumbs', s.devTn + ' / ' + s.thN + ' \u00B7 ' + _upFmtBytes(s.devT));
    set('rtStatLib', s.cloudN + ' files, ' + folders.length + ' folders \u00B7 ' + _upFmtBytes(libBytes));
    set('rtStatMain', s.cloudN + ' \u00B7 ' + _upFmtBytes(s.main));
    set('rtStatCloud', (s.cloudN + s.thN + lib) + ' files \u00B7 ' + unk + _upFmtBytes(s.main + s.th + libBytes));
...
async function retrieveRun() {
  ...
  const ui = (t, s, pct) => { if (title) title.textContent = t; if (status) status.textContent = s || ''; _rtSetSeg(pct); };
...
async function uploadRefreshStats() {
  ...
  try {
    ...
    set('upStatVault', (s.cloudN + s.thN + libFiles) + ' files \u00B7 ' + unk + F(s.main + s.th + cloudLib));

    const last = parseInt(localStorage.getItem('pv_last_upload') || '0', 10);
    set('upLast', last ? 'Last upload: ' + new Date(last).toLocaleString() : 'Not uploaded from this device yet');
