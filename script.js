    ...
    (function(pid) {
      ...
      card.addEventListener('touchmove', function(e) {
        ...
        if (!locked && (Math.abs(dx) > 8 || Math.abs(dy) > 8)) {
          ...
          if (locked === 'h') { dragging = true; if (lpTimer) { clearTimeout(lpTimer); lpTimer = null; } }
          else if (lpTimer) { clearTimeout(lpTimer); lpTimer = null; card.style.transform = ''; }
        }
...
async function _vcDerivFromSource(src, sw, sh) {
  const thumb = await _vcToJpeg(_vcDraw(src, sw, sh, _VC.THUMB_PX), 0.72);
  return { thumb };
}
