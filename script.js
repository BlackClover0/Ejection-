/* ════════════════════════════════════════════════════════════════════════
   ...
   regrouped, for that reason.
     ...
     SECTION 1 — VAULT UNLOCK .................. line 1
       (lock screen removed — sessionPin is a fixed constant, app opens
       directly; see the section comment below for why)
     ...
     SECTION 6 — EVERYTHING ELSE ............... line ~4611
       ...
       rendering, confirm modal, PWA install —
...
async function _autoRestoreFromCloud() {
  ...
  if (_vcLibCleared()) return;      // library was cleared on this device — stays empty until Retrieve
...
function _setCloudBarBusy(busy) {
  ...
  const rb = document.getElementById('uploadRunBtn');
  if (rb) { rb.disabled = !!busy; rb.style.opacity = busy ? '0.55' : '1'; }
  if (!busy && typeof uploadRefreshStats === 'function') uploadRefreshStats();
...
function openBinModal() {
  ...
  clearTimeout(_bm._closeT);            // a pending close must never shut a panel that was just re-opened
  window._binSettleAt = Date.now() + 1000;  // thumbnails start loading 1s after open, once the 700ms open animation has settled
  _renderBinItems();
  _updateBinBadge();
...
function openDisplaySettings() {
  ...
  // A pending close (see closeDisplaySettings) must never fire after a
  // re-open — without this, reopening quickly during the 280ms close window
  // left the old timeout alive, and it would hide the page out from under
  // you a moment later even though you'd just opened it.
  clearTimeout(panel._closeT);
  sheet.style.display = 'block';
  if (_dsReduceMotion) {
    panel.style.animation = 'none';
  } else {
    _pmForceRestart(panel);
    panel.style.animation = 'afsSlideUp 0.7s cubic-bezier(0.22,1,0.36,1) both';
  }
  _dsRefreshUI();
}
...
function closeDisplaySettings() {
  ...
  _pmForceRestart(panel);
  panel.style.animation = 'afsSlideOut 0.4s cubic-bezier(0.22,1,0.36,1) forwards';
  panel._closeT = setTimeout(() => {
    sheet.style.display = 'none';
...
function _dsRefreshUI() {
  ...
  const themeState = document.getElementById('dsThemeStateLabel');
  if (themeState) themeState.textContent = (_dsLightMode ? 'Light' : 'Dark') + ' is on';
...
/* Paints the filled (gold) portion of the brightness track behind the native
   <input type=range>, since the browser won't do a partial-fill gradient
   for us — the visual %-filled cue relies on this running on every change. */
function _dsUpdateSliderFill() {
  const slider = document.getElementById('dsBrightnessSlider');
  if (!slider) return;
  const min = parseFloat(slider.min) || 0;
  const max = parseFloat(slider.max) || 100;
  const pct = ((_dsBrightness - min) / (max - min)) * 100;
  slider.style.background = `linear-gradient(90deg,#8a744c 0%,var(--gold) ${pct}%,rgba(255,255,255,0.08) ${pct}%,rgba(255,255,255,0.08) 100%)`;
}
...
function openFolderDetailPage(folderId) {
  ...
  if (!document.getElementById('_fdpKf')) {
    ...
    s.textContent = `
      ...
      @keyframes fdpFlipSideways { from{opacity:0;transform:perspective(1200px) rotateY(-90deg)} to{opacity:1;transform:none} }
      @keyframes fdpFlipSidewaysOut { from{opacity:1;transform:none} to{opacity:0;transform:perspective(1200px) rotateY(90deg)} }
  ...
  // Animate in — opening a subfolder (from any route: its tile, sidebar, search) flips
  // sideways like a card. Re-opening the same page to refresh it (sort, rename, etc.)
  // is not a navigation, so it keeps the previous animation instead of flipping again.
  const alreadyOpen = !!(overlay.style.display === 'flex' || overlay.style.display === '');
  const isRefresh   = alreadyOpen && overlay._fdpFolderId === folderId;
  const flipIn      = !!f.parentId && !isRefresh;
  overlay.style.animation = 'none';
  void overlay.offsetWidth;
  overlay.style.animation = flipIn
    ? 'fdpFlipSideways 0.6s cubic-bezier(0.22,1,0.36,1) both'
    : alreadyOpen
  ...
  overlay.innerHTML = `
    ...
    <div style="flex:1;min-height:0;overflow-y:auto;-webkit-overflow-scrolling:touch;box-sizing:border-box;">
      <!-- header -->
      <div style="padding:calc(env(safe-area-inset-top, 0px) + var(--header-top-gap)) 18px 0;background:#000000;">
...
function closeFolderDetailPage() {
  ...
  // A subfolder page closes with the matching flip sideways (500ms, from the motion
  // picker); top-level folders keep the original slide-out (280ms).
  const curFolder = (typeof folders !== 'undefined') ? folders.find(x => x.id === overlay._fdpFolderId) : null;
  const flipOut = !!(curFolder && curFolder.parentId);
  overlay.style.animation = flipOut
    ? 'fdpFlipSidewaysOut 0.5s cubic-bezier(0.4,0,0.7,1) forwards'
    : 'fdpSlideOut 0.28s cubic-bezier(0.22,1,0.36,1) forwards';
  setTimeout(() => {
...
function _buildCollectionPageDOM(overlay, type, meta) {
  ...
  overlay.innerHTML = `
    <!-- header -->
    <div style="flex-shrink:0;padding:calc(env(safe-area-inset-top, 0px) + var(--header-top-gap)) 18px 0;background:#000000;">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
...
function _afsDeleteAllFolders() {
  ...
  showConfirmModal(
    ...
    () => {
      ...
      // Snapshot every folder being deleted so restore-from-bin can recreate it.
      const deletedFolderSnapshots = folders.map(x => ({ id: x.id, name: x.name, parentId: x.parentId }));
      const affectedPhotos = photos.filter(p => allFolderIds.has(p.folder));
      affectedPhotos.forEach(p => {
        p.trashedAt = Date.now();
        p._deletedFolders = deletedFolderSnapshots;
        if (p.storage === 'idb') _removeFromPendingQueue(p.id);
      });
...
function deleteFolder(id) {
  ...
  // Snapshot the folder(s) being deleted (id/name/parentId) so a later
  // restore-from-bin can recreate them instead of just dumping the photo on Home.
  const deletedFolderSnapshots = folders.filter(x => allIds.has(x.id))
    .map(x => ({ id: x.id, name: x.name, parentId: x.parentId }));
...
function tgClearConfig() {
  showConfirmModal(
    'Disconnect Pcloud?',
    'Your files on Pcloud will NOT be deleted.',
    'Disconnect', '#ff4444',
    () => {
...
function _tgRefreshStatusUI() {
  ...
  // Pcloud page (dial / Online-Offline pill / state title)
  const cmScreen = document.getElementById('cmScreen');
  if (cmScreen) {
    const on = isPcloudEnabled();
    cmScreen.classList.toggle('cm-on', on);
    const cmState = document.getElementById('cmState');
    const cmChip  = document.getElementById('cmChipText');
    if (cmState) cmState.textContent = on ? 'Connected' : 'Not connected';
    if (cmChip)  cmChip.textContent  = on ? 'Online' : 'Offline';
  }
  _pcRefreshAppKeyUI();
}

// "How setup works" floating window on the Pcloud page
function _pcInfo(open) {
  const m = document.getElementById('cmModal');
  if (m) m.classList.toggle('cm-open', !!open);
}
...
async function _tgSaveManifest() {
  // "Clear library" only empties THIS device. Before any write, bring the cloud library back, so a
  // cleared device can never overwrite the real manifest with a short one. Fails (safely) if offline.
  if (_vcLibCleared()) await _vcHealLibrary();
  const manifest = {
    ...
    photos: photos.map(p => ({
      ...
      fav: p.fav || undefined,
      thId: p.thId || undefined,   // encrypted thumbnail in Pcloud
      thSize: p.thSize || undefined,
    })),
    folders
  };
  ...
  try { _pcFlushDeleteQueue(); } catch (e) {}   // retry any cloud deletes that were waiting
  return fileId;
}
...
async function _tgPushAllCore() {
  ...
  for (const id of snapshot) {
    ...
    try {
      ...
      // The thumbnail goes up too, so any device (or a reinstalled APK) can rebuild the gallery.
...
/* Shared by the Pcloud / Upload / Retrieve / Device Storage / Cloud Storage
   pages. These pages' open/close animation is a plain CSS @keyframes
   animation, which only plays when the class that triggers it actually
   changes. If you tap to reopen a page while it's still mid-close (or close
   it while it's still mid-open), the relevant class can end up unchanged —
   so the browser silently skips the animation instead of replaying it. That
   was the "opens too fast" / "sometimes glitches" bug: it wasn't playing a
   bad animation, it just wasn't playing one at all. This forces a fresh
   restart every time, from whichever state it's currently in, by clearing
   the animation for one frame (with a forced reflow so the browser actually
   registers the reset) right before the class change that starts it again. */
function _pmForceRestart(el) {
  if (!el) return;
...
function closeCloudModal() {
  ...
  m._closeT = setTimeout(() => { m.classList.remove('open', 'closing'); }, 500);
}
...
function startCloudPolling() {
  ...
  _pollInterval = setInterval(async () => {
    if (!isCloudEnabled() || _cloudSyncing || document.hidden || _vcLibCleared()) return;
...
function _updateStoragePill() {
  ...
  label.textContent = _formatSize(totalBytes);
  if (sub) sub.textContent = _formatSize(totalBytes) + ' used';
  const hsl = document.getElementById('headerStorageLabel');
  if (hsl) hsl.textContent = _formatSize(totalBytes);
...
function openStorageBreakdown() {
  ...
  document.getElementById('sbTotalLabel').textContent = _formatSize(totalBytes);
...
function load() {
  // ── Photos are still session-only — rebuilt fresh on every app open rather
  // than persisted, same as before the lock screen was removed. What changed:
  // we no longer blindly wipe IndexedDB here. Any file that was added but not
  // yet uploaded to Pcloud is reconstructed from the pending-uploads queue
  // below, so it is still there and still waiting for the next tap on Upload.
  photos = [];
  try {
...
/* ── Cloud delete queue: Pcloud file ids that still have to be removed (offline, not connected, or a
     failed call). Retried on every later delete and on every manifest save, so nothing is left behind. ── */
function _pcDelQueueGet() { try { return JSON.parse(localStorage.getItem('pv_pc_del_queue') || '[]'); } catch (e) { return []; } }
function _pcDelQueueSet(a) { try { localStorage.setItem('pv_pc_del_queue', JSON.stringify(a)); } catch (e) {} }
function _pcDelQueueAdd(ids) {
  const q = _pcDelQueueGet();
  ids.forEach(i => { i = String(i); if (i && !q.includes(i)) q.push(i); });
  _pcDelQueueSet(q);
}
let _pcDelChain = Promise.resolve();
function _pcFlushDeleteQueue() {
  _pcDelChain = _pcDelChain.then(async () => {
    if (!isDeleteEnabled()) return;
    const q = _pcDelQueueGet();
    if (!q.length) return;
    const done = new Set();
    for (const id of q) {
      try { await _pcDeleteFile(id); done.add(id); }
      catch (e) { if (/not found/i.test(e && e.message || '')) done.add(id); }   // already gone = done
    }
    _pcDelQueueSet(_pcDelQueueGet().filter(id => !done.has(id)));
  }).catch(() => {});
  return _pcDelChain;
}

/* Permanently removes EVERYTHING belonging to a photo/video: the entry (id), the cached thumbnail and
   original on this device, the in-memory thumbnail, the encrypted blob, the pending-upload entry, and
   the main file + thumbnail in Pcloud. Returns { queued } = cloud files that could not be removed yet. */
async function _deletePhotosFromCloud(targets) {
  const cloudIds = [];
  for (const p of targets) {
    try { _vcForget(p.id); } catch (e) {}                       // thumb + orig + origmeta in the device cache
    const tc = _thumbCache[p.id];
    if (tc) {
      try { if (tc.url) URL.revokeObjectURL(tc.url); if (tc.posterUrl) URL.revokeObjectURL(tc.posterUrl); } catch (e) {}
      delete _thumbCache[p.id];
    }
    if (p.encId) idbDelete(p.encId).catch(() => {});             // encrypted blob waiting on this device
    try { _removeFromPendingQueue(p.id); } catch (e) {}
    [p.pcFileId, p.thId].forEach(fid => {                        // main file + thumbnail in Pcloud
      if (fid) { try { _vcLinkForget(fid); } catch (e) {} cloudIds.push(String(fid)); }
    });
  }
  if (!cloudIds.length) return { queued: 0 };
  _pcDelQueueAdd(cloudIds);
  await _pcFlushDeleteQueue();
  const pending = new Set(_pcDelQueueGet());
  return { queued: cloudIds.filter(id => pending.has(id)).length };
}
...
function emptyBin() {
  ...
  showConfirmModal(
    'Empty Bin?',
    `Permanently delete all ${count} item(s) from the bin? This cannot be undone.`,
    'Delete All', '#ff4444',
    () => { _emptyBinConfirmed(); }
  );
}

async function _emptyBinConfirmed() {
  ...
  const hasRemote = toDelete.some(p => p.pcFileId || p.thId);
  try {
    if (hasRemote) showToast('🗑️ Deleting…');
    const r = await _deletePhotosFromCloud(toDelete);
    if (r.queued > 0) showToast(`🗑️ Bin emptied — ${r.queued} cloud file(s) will be removed when Pcloud is reachable`);
    else showToast(`🗑️ Bin emptied — ${toDelete.length} item(s) permanently deleted`);
  } catch (e) {
    showToast('⚠️ Bin emptied — cloud cleanup will retry: ' + (e.message || ''));
    console.warn('emptyBin cleanup error:', e);
  }
}
...
function closeBinModal() {
  ...
  m.classList.add('closing');
  clearTimeout(m._closeT);
  m._closeT = setTimeout(() => { m.classList.remove('open', 'closing'); }, 350);
}
...
function restoreFromBin(id) {
  ...
  delete p.trashedAt;
  // If the folder it belonged to (p.folder — not p.folderId, which photos never
  // have) no longer exists, recreate it (and its parent, if that was deleted
  // too) from the snapshot taken when the folder was deleted, instead of
  // silently dropping the photo onto Home.
  if (p.folder && !folders.find(f => f.id === p.folder)) {
    const snaps = p._deletedFolders || [];
    // Recreate parents before children so a subfolder's parentId always resolves.
    const byParentFirst = [...snaps].sort((a, b) => (a.parentId ? 1 : 0) - (b.parentId ? 1 : 0));
    byParentFirst.forEach(s => {
      if (s.id && !folders.find(f => f.id === s.id)) {
        folders.push({ id: s.id, name: s.name, parentId: s.parentId });
      }
    });
    // Still missing (no snapshot available, e.g. older bin entry) → fall back to Home.
    if (!folders.find(f => f.id === p.folder)) p.folder = 'home';
  }
  delete p._deletedFolders;
  photos.push(p);
  if (p.storage === 'idb' && p.encId) { _addToPendingQueue(p); _updateCloudBar(true); }
  _saveTrashed();
  save(); render();
  if (typeof renderFolders === 'function') renderFolders();
  // Remove the whole grid cell (wrapper), not just the inner card — each cell in
  // #binItemList is the wrapper div, so removing only the card left an empty hole
  // behind instead of the rest of the grid reflowing to fill the gap, the way
  // swipe-to-delete already does it correctly.
  const card = document.querySelector('#binItemList [data-pid="' + id + '"]');
  if (card) {
    const wrapper = card.parentNode;
    const list = document.getElementById('binItemList');
    if (wrapper && wrapper !== list) {
      // Same smooth collapse feel as swipe-to-delete
      const h = wrapper.offsetHeight;
      card.style.transition = 'transform 0.2s cubic-bezier(0.4,0,1,1), opacity 0.2s ease';
      card.style.transform = 'scale(0.85)';
      card.style.opacity = '0';
      wrapper.style.overflow = 'hidden';
      wrapper.style.height = h + 'px';
      wrapper.offsetHeight; // force reflow
      wrapper.style.transition = 'height 0.2s cubic-bezier(0.4,0,0.2,1)';
...
// permanentDelete(id) was removed here: it was fully written but never wired to
// any button. The two real permanent-delete paths (swipe-to-delete in
// _renderBinItems and _emptyBinConfirmed) now both have the same correct,
// awaited cloud-delete + accurate-toast behavior it had, so it was redundant.
...
/* Default "Image" background option for the Bin page — a self-contained dark
   velvet/marble-style texture (inline SVG data URI) that matches the app's
   black/gold theme. No network request needed, so it always loads. */
const _BIN_BG_IMG = "data:image/svg+xml;utf8," + encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" width="800" height="1200" viewBox="0 0 800 1200">' +
    '<defs>' +
      '<radialGradient id="g1" cx="50%" cy="-6%" r="75%">' +
        '<stop offset="0%" stop-color="#c8ad7a" stop-opacity="0.30"/>' +
        '<stop offset="65%" stop-color="#c8ad7a" stop-opacity="0"/>' +
      '</radialGradient>' +
      '<radialGradient id="g2" cx="50%" cy="112%" r="70%">' +
        '<stop offset="0%" stop-color="#c8ad7a" stop-opacity="0.14"/>' +
        '<stop offset="60%" stop-color="#c8ad7a" stop-opacity="0"/>' +
      '</radialGradient>' +
      '<linearGradient id="g3" x1="0" y1="0" x2="0" y2="1">' +
        '<stop offset="0%" stop-color="#17130c"/>' +
        '<stop offset="100%" stop-color="#0a0a0c"/>' +
      '</linearGradient>' +
    '</defs>' +
    '<rect width="800" height="1200" fill="url(#g3)"/>' +
    '<rect width="800" height="1200" fill="url(#g1)"/>' +
    '<rect width="800" height="1200" fill="url(#g2)"/>' +
  '</svg>'
);
...
// ── Bin long-press-to-restore safety net ──
// On some devices, taking a screenshot (button combo, edge/knuckle gesture,
// or a 3-finger swipe) never fires touchend/touchcancel on the element the
// finger happens to be resting on — so the pending "hold to restore" timer
// still completes and silently restores the photo out of the bin. We track
// every active long-press timer here and cancel all of them the moment the
// page loses visibility/focus (which system screenshot UI reliably triggers),
// so a screenshot can never be mistaken for an intentional long press.
window._binLongPressSet = window._binLongPressSet || new Set();
function _binLPRegister(entry) { window._binLongPressSet.add(entry); }
function _binLPUnregister(entry) { if (entry) window._binLongPressSet.delete(entry); }
if (!window._binLPGuardInstalled) {
  window._binLPGuardInstalled = true;
  var _cancelAllBinLongPresses = function() {
    window._binLongPressSet.forEach(function(entry) { entry.cancel(); });
    window._binLongPressSet.clear();
  };
  document.addEventListener('visibilitychange', function() {
    if (document.hidden) _cancelAllBinLongPresses();
  });
  window.addEventListener('blur', _cancelAllBinLongPresses);
}
...
function _renderBinItems() {
  ...
  trashedPhotos.forEach((p, i) => {
    ...
    // Same springy press feel as the folder/subfolder tiles (.fdp-card): a quick scale-down
    // on touch, and a slight overshoot back to full size on release.
    card.style.cssText = 'position:relative;width:100%;aspect-ratio:1/1;border-radius:14px;overflow:hidden;background:var(--card);border:1px solid rgba(255,255,255,0.075);box-shadow:0 8px 20px -12px rgba(0,0,0,0.7);cursor:pointer;transition:transform 0.16s cubic-bezier(.34,1.56,.64,1);-webkit-tap-highlight-color:transparent;-webkit-touch-callout:none;-webkit-user-select:none;user-select:none;';
    card.addEventListener('contextmenu', function(e) { e.preventDefault(); });

    // Single tap → lightbox; long press (600ms) → restore; horizontal drag → swipe to delete
    (function(pid) {
      var lpTimer = null, lpFired = false, startX = 0, startY = 0;
      var locked = null;      // null (undecided) | 'h' (swipe) | 'v' (scroll/ignore)
      var dragging = false, dragDX = 0;
      var SWIPE_COMMIT = 90;  // px to drag before release counts as delete
      var lpEntry = null;     // registered with the global long-press safety net while pending

      card.addEventListener('touchstart', function(e) {
        // A second (or third) finger landing on the card means this is a
        // multi-touch system gesture (e.g. a 3-finger screenshot swipe), not
        // an intentional single-finger long press — bail out immediately.
        if (e.touches.length > 1) {
          _binLPUnregister(lpEntry); lpEntry = null;
          if (lpTimer) { clearTimeout(lpTimer); lpTimer = null; }
          card.style.transform = '';
          return;
        }
        lpFired = false; locked = null; dragging = false; dragDX = 0;
        startX = e.touches[0].clientX;
        startY = e.touches[0].clientY;
        card.style.transition = 'transform 0.16s cubic-bezier(.34,1.56,.64,1)';
        ...
        lpEntry = { cancel: function() {
          clearTimeout(lpTimer); lpTimer = null;
          card.style.transform = '';
          card.style.opacity = '1';
        }};
        _binLPRegister(lpEntry);
      }, {passive: true});

      card.addEventListener('touchmove', function(e) {
        if (e.touches.length > 1) {
          _binLPUnregister(lpEntry); lpEntry = null;
          if (lpTimer) { clearTimeout(lpTimer); lpTimer = null; card.style.transform = ''; }
          return;
        }
        ...
        // swipe or a vertical scroll — avoids fighting the page scroll.
        if (!locked && (Math.abs(dx) > 8 || Math.abs(dy) > 8)) {
          locked = Math.abs(dx) > Math.abs(dy) * 1.5 ? 'h' : 'v';
          if (locked === 'h') { dragging = true; if (lpTimer) { clearTimeout(lpTimer); lpTimer = null; _binLPUnregister(lpEntry); lpEntry = null; } }
          else if (lpTimer) { clearTimeout(lpTimer); lpTimer = null; card.style.transform = ''; _binLPUnregister(lpEntry); lpEntry = null; }
        }

        if (locked === 'h' && dragging) {
          e.preventDefault();
          dragDX = dx;
          // Follow the finger 1:1 — no transition while actively dragging — with a
          // gentle scale/fade as it travels, instead of jumping straight to "deleted".
          card.style.transition = 'none';
          var scale = 1 - Math.min(Math.abs(dx) / 900, 0.06);
          card.style.transform = 'translateX(' + dx + 'px) scale(' + scale + ')';
          card.style.opacity = String(Math.max(0.3, 1 - Math.abs(dx) / 240));
        }
      }, {passive: false});
      ...
      card.addEventListener('touchend', function(e) {
        ...
        if (dragging) {
          dragging = false;
          if (Math.abs(dragDX) > SWIPE_COMMIT) {
            // Committed — finish flying off screen from wherever the finger let go
            ...
            setTimeout(function() {
              ...
              if (idx >= 0) {
                ...
                if (typeof _updateBinBadge === 'function') _updateBinBadge();
                var hasRemote = !!(p.pcFileId || p.thId);
                if (hasRemote) showToast('🗑️ Deleting…');
                _deletePhotosFromCloud([p]).then(function(r) {   // thumbnail, caches, blob + Pcloud files
                  if (r && r.queued > 0) showToast('🗑️ Deleted here — cloud copy will be removed when Pcloud is reachable');
                  else if (hasRemote) showToast('🗑️ Permanently deleted');
                }).catch(function(e) {
                  showToast('⚠️ Deleted here — cloud cleanup will retry: ' + (e && e.message || ''));
                });
                if (!hasRemote) showToast('🗑️ Permanently deleted');
              }
            }, 200);
          } else {
            // Didn't drag far enough — spring back to place
            card.style.transition = 'transform 0.32s cubic-bezier(.34,1.56,.64,1), opacity 0.24s ease';
            card.style.transform = '';
            card.style.opacity = '1';
          }
          return;
    ...
    // Thumbnail: device cache first; loaded in a throttled queue once the open animation has settled
    jobs.push(() => _binLoadThumb(card, p, shimmer));

    cardWrap.appendChild(card);
    list.appendChild(cardWrap);
  });

  // Start thumbnails once the bin has settled, one at a time with a small gap between
  // each — loading them all at once (or in a big parallel burst) is what was causing
  // the stutter/flicker right after the bin opened.
  setTimeout(() => {
    let next = 0;
    const worker = async () => {
      while (next < jobs.length && gen === _binThumbGen) {
        const j = jobs[next++];
        try { await j(); } catch (e) {}
        await new Promise(r => setTimeout(r, 40)); // brief pause so thumbs trickle in instead of popping in together
      }
    };
    worker();
  }, Math.max(0, (window._binSettleAt || 0) - Date.now()));
}

var _binThumbGen = 0;

async function _binLoadThumb(card, p, shimmer) {
  if (!card.isConnected) return;
  let ok = false;
  try { ok = await _vcPaint(card, p, { shimmer, first: true, z: 1, badge: 28 }); } catch (e) {}
  if (!ok && card.isConnected) await _binLegacyThumb(card, p, shimmer);
}

/* Fallback (no cached/derived thumbnail could be made): decrypt the file itself. Doesn't cache the original. */
async function _binLegacyThumb(card, p, shimmer) {
  try {
    let encBlob;
    if (p.storage === 'cloud' && p.pcFileId) {
      encBlob = await fetchCloudBlob(p, true);
    } else if (p.storage === 'idb' && p.encId) {
      encBlob = await idbGet(p.encId);
    ...
    if (isVideo) {
      ...
      // The image branch below already revokes its blob URL after 30s; this branch
      // never did, so every video thumbnail rendered this way leaked for the rest
      // of the session. Match the same revoke-after-30s cleanup here.
...
function _updateBinBadge() {
  ...
  // Gallery "More" section row — always shown, mirrors the other media-type counts
  const ccBadge = document.getElementById('ccBinBadge');
  if (ccBadge) ccBadge.textContent = trashedPhotos.length;
  // Gold pill next to the "BIN / TRASH" title inside the Bin page itself
  const badge2 = document.getElementById('binCountBadge2');
  if (badge2) {
    if (trashedPhotos.length > 0) {
      badge2.style.display = 'inline-block';
      badge2.textContent = trashedPhotos.length;
    } else {
...
function _saveTrashed() {
  try {
    // Persist cloud AND device-only trashed items across sessions, so items
    // in the Bin survive a refresh/reopen until permanently deleted. Device-only
    // items' actual encrypted bytes live in IndexedDB (idb, keyed by encId) or
    // are embedded inline (local, via encData) — either way that data isn't
    // touched here, only the Bin's metadata entry for it is persisted.
    const persistable = trashedPhotos.filter(p =>
      (p.storage === 'cloud' && p.pcFileId) ||
      (p.storage === 'idb' && p.encId) ||
      (p.storage === 'local' && p.encData)
    );
...
async function _encryptAndSaveBatch({ files, folderId, pendingType }) {
  ...
  for (const en of entries) {
    try {
      ...
      _vcSeedFromPlain(en.id, en.file, _getMediaType(en.file)).catch(() => {});  // thumbnail, in background
      const photo = {
        id: en.id,
...
async function openLightbox(startIndex, list) {
  ...
  window._lbSettleAt = Date.now() + 1000;  // heavy decrypt/full-quality load waits 1s for the open transition to settle
  ...
  // Fill a slide: local thumbnail first (instant), full-resolution swaps in behind it.
  ...
  // Warm the neighbours (thumbnails are local, so swiping never waits)
...
function closeLightbox() {
  ...
  setTimeout(() => {
    lb.classList.remove('lb-visible', 'lb-closing', 'open');
    decryptedUrls.forEach(u => URL.revokeObjectURL(u));
    decryptedUrls = [];
    const track = document.getElementById('sliderTrack');
...
function _lbUpdateUI() {
  ...
  // In the Bin, the delete row restores instead of permanently deleting —
  // permanent delete only happens via swipe-to-delete or Empty Bin on the Bin page.
  const inBin = typeof trashedPhotos !== 'undefined' && trashedPhotos.some(x => x.id === p.id);
  const sheetLbl  = document.getElementById('lbSheetDeleteLabel');
  const flatLbl   = document.getElementById('lbFlatDeleteLabel');
  const sheetIcon = document.getElementById('lbSheetDeleteIcon');
  const sheetGlyph = document.getElementById('lbSheetDeleteIconGlyph');
  if (sheetLbl) { sheetLbl.textContent = inBin ? 'Restore' : 'Delete'; sheetLbl.style.color = inBin ? 'var(--gold)' : '#ff6b6b'; }
  if (flatLbl)  flatLbl.textContent  = inBin ? 'Restore' : 'Delete';
  if (sheetIcon) sheetIcon.style.background = inBin ? 'rgba(200,173,122,0.16)' : 'rgba(239,68,68,0.13)';
  if (sheetGlyph) { sheetGlyph.className = inBin ? 'fas fa-rotate-left' : 'fas fa-trash'; sheetGlyph.style.color = inBin ? 'var(--gold)' : '#ff6b6b'; }
}
...
function lbAction_delete() {
  ...
  // Opened from the Bin → permanent delete only happens via swipe-to-delete or
  // Empty Bin, both on the Bin page itself. From here, offer Restore instead.
  if (typeof trashedPhotos !== 'undefined' && trashedPhotos.some(x => x.id === p.id)) {
    showConfirmModal(
      'Restore from Bin?',
      '"' + (p.name || 'this file') + '" will be moved back out of the Bin.',
      'Restore', '#c8ad7a',
      () => {
...
function showConfirmModal(title, message, confirmLabel, confirmColor, onConfirm) {
  ...
  if (!el) {
    ...
    el.id = '_confirmModal';
    el.style.cssText = 'display:none;position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,0.65);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);align-items:center;justify-content:center;padding:24px;box-sizing:border-box;font-family:\'Nunito\',sans-serif;';
    el.innerHTML = `
      <div id="_confirmBox" style="background:var(--card,#17161a);border:1px solid var(--border,rgba(255,255,255,0.09));border-radius:20px;padding:26px 22px 20px;max-width:320px;width:100%;box-shadow:0 24px 60px rgba(0,0,0,0.8), 0 0 0 1px rgba(200,173,122,0.06);">
        <div style="width:38px;height:38px;border-radius:50%;background:rgba(200,173,122,0.12);border:1px solid rgba(200,173,122,0.3);display:flex;align-items:center;justify-content:center;margin-bottom:14px;">
          <i class="fas fa-trash-can" style="font-size:14px;color:var(--gold,#c8ad7a);"></i>
        </div>
        <div id="_confirmTitle" style="font-size:17px;font-weight:800;color:#fff;margin-bottom:8px;letter-spacing:0.01em;"></div>
        <div id="_confirmMsg" style="font-size:13px;color:rgba(255,255,255,0.5);line-height:1.55;margin-bottom:22px;font-weight:600;"></div>
        <div style="display:flex;gap:10px;">
          <button id="_confirmCancel" style="flex:1;padding:12px;border-radius:12px;border:1px solid var(--border,rgba(255,255,255,0.09));background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.7);font-size:14px;font-weight:700;cursor:pointer;font-family:'Nunito',sans-serif;">Cancel</button>
          <button id="_confirmOk" style="flex:1;padding:12px;border-radius:12px;border:none;font-size:14px;font-weight:700;cursor:pointer;color:#fff;font-family:'Nunito',sans-serif;"></button>
        </div>
      </div>`;
...
/* ════════════════════════════════════════════════════════════════════════
     SECTION 6 — VAULT CACHE (thumbnails, library index)

     Every file is stored in Pcloud as TWO encrypted pieces:
        original   hope_<id>_<rid>_<name>.enc   (the real file)
        thumbnail  hope_<id>_th.enc             (~400px JPEG, tens of KB)
     and hope_manifest.json lists them all (ids, folders, thumbnail ids).
     The device keeps a cache of the thumbnails + the library index in
...
const _VC = {
  THUMB_PX: 640,                    // longest side of the grid thumbnail
};
let _vcReady = false;
...
/* read-modify-write in ONE transaction so parallel writes can't clobber each other */
function _vcMergeRec(id, patch) {
  return _vcDb().then(db => new Promise((res, rej) => {
    ...
    tx.oncomplete = () => { res(next); _vcScheduleExtra(); };
...
const _vcNet = _vcLimiter(6);   // small thumbnail downloads

function _vcKind(p) { return (_getMediaType(p) === 'video' || p.mediaType === 'video') ? 'video' : 'image'; }

/* ── thumbnail generation (runs on the device — Pcloud only ever sees ciphertext) ── */
...
async function _vcDerivFromSource(src, sw, sh) {
  // Higher resolution + quality than before — thumbnails looked soft once tiles
  // got bigger (fewer grid columns) or on high-density screens, since 400px was
  // being upscaled past its native detail.
  const thumb = await _vcToJpeg(_vcDraw(src, sw, sh, _VC.THUMB_PX), 0.85);
  return { thumb };
}
...
/* plain file → encrypted thumbnail stored on the device */
...
async function _vcPutDerivs(p, rec) {
  if (!p.thId) { p.thId = await _vcUploadRetry(rec.t, 'hope_' + p.id + '_th.enc'); p.thSize = rec.t.size; }
}
...
async function _vcUploadDerivs(p, encOriginal) {
  ...
  if (rec && rec.t) await _vcPutDerivs(p, rec);
}
...
/* Files uploaded before this feature (or whose thumb upload failed) get their
   thumbnail uploaded later, once it exists on this device. */
...
async function _vcRepairCloudDerivs() {
  ...
  try {
    for (const p of photos.slice()) {
      if (p.storage !== 'cloud' || p.thId) continue;
...
async function _vcWarmThumbs() {
  if (_vcWarming || !isPcloudEnabled() || _vcThumbsCleared()) return;
...
function _vcBuildOnce(p) {
  ...
  const job = (async () => {
    ...
    if (rec && p.storage === 'cloud' && !p.thId) _vcQueueRepair();
    return rec;
...
/* ── thumbnail → blob URL (memory → device → Pcloud → build) ── */
const _vcMaps = { t: new Map() };
const _vcCaps = { t: 3000 };
...
/* ── lightbox: local thumbnail first, full quality swaps in behind it ── */
async function _vcFillSlide(idx, autoplay, wantFull) {
  ...
  if (!slide.dataset.quick) {
    ...
    let q = null;
    try { q = await _vcUrl('t', p, false); } catch (e) {}
    if (q && slide.isConnected && !slide.dataset.fullDone) {
      const img = document.createElement('img');
      img.src = q; img.draggable = false;
      img.style.cssText = 'width:100%;height:100%;object-fit:contain;';   // same box the full file will use → no size jump on swap
  ...
  const upgrade = (async () => {
    try {
      // Always let the open transition finish before starting the heavy decrypt —
      // starting it immediately (as before) was what froze the spin-in animation
      // and left the screen sitting on black while a file decrypted.
      const wait = (window._lbSettleAt || 0) - Date.now();
      ...
      if (!slide.isConnected) return;
      // Layer the full file over the thumbnail, then drop the thumbnail — never an empty frame in between
      slide.appendChild(node);
      slide.dataset.fullDone = '1';
      const dropOld = () => { Array.from(slide.children).forEach(c => { if (c !== node) c.remove(); }); };
      if (isVideo) { node.addEventListener('loadeddata', dropOld, { once: true }); setTimeout(dropOld, 800); }
...
async function _prefetchAdjacentSlides(idx) {
  // Thumbnails are small and cached → warm 3 slides each way; full quality only for the immediate neighbours.
...
/* ── caches: originals you opened (still encrypted) kept on the device, no size limit ── */
...
function _vcScheduleIndexSave() { clearTimeout(_vcIdxTimer); _vcIdxTimer = setTimeout(_vcSaveIndexNow, 800); _vcScheduleExtra(); }
...
async function _vcLoadIndexIntoPhotos() {
  ...
  } finally { _vcReady = true; _vcScheduleExtra(); _vcDropLegacyPreviews(); }
}
document.addEventListener('visibilitychange', () => {
...
function _vcMergeManifest(manifest) {
  ...
  for (const cp of cloudPhotos) {
    if (!existingIds.has(cp.id)) { photos.push(cp); existingIds.add(cp.id); added++; }
      ...
      const lp = localById.get(cp.id);   // keep the local record, but learn the cloud thumbnail id
      if (lp && lp.storage === 'cloud') { if (!lp.thId && cp.thId) lp.thId = cp.thId; if (!lp.thSize && cp.thSize) lp.thSize = cp.thSize; }
    }
  }
  ...
  _vcSetLibCleared(false);      // the library is back on this device
  return { added, removed, total: cloudPhotos.length };
}

/* which thumbnails are already saved on this device */
async function _vcLocalState() {
  const hasT = new Set(), szT = new Map();
  ...
  await new Promise((res, rej) => {
    ...
    rq.onsuccess = () => {
      const c = rq.result;
      if (!c) return res();
      if (c.value && c.value.t) { hasT.add(c.key); szT.set(c.key, c.value.t.size || 0); }
      c.continue();
    };
...
/* storage numbers for the Upload and Retrieve pages */
async function _vcStorageStats() {
  const { hasT, szT } = await _vcLocalState();
  let cache = 0;
  try { (await _vcTx('origmeta', 'readonly', st => st.getAll())).forEach(m => { cache += (m && m.size) || 0; }); } catch (e) {}
  const cloud = photos.filter(p => p.storage === 'cloud');
  const waiting = photos.filter(_isWaitingForUpload);
  const s = {
    cloudN: cloud.length, main: 0,
    waitingN: waiting.length, waiting: waiting.reduce((a, p) => a + (p.size || 0), 0),
    thN: 0, th: 0, thUnk: 0,                                     // in Pcloud
    devTn: 0, devT: 0, cache,                                    // on this device
    getN: 0, get: 0, getUnk: 0,                                  // still to download
    noThumb: 0,
  };
  for (const p of cloud) {
    s.main += p.size || 0;
    if (!p.thSize && szT.has(p.id) && p.thId) p.thSize = szT.get(p.id);   // learn sizes from what we hold
    if (p.thId) { s.thN++; if (p.thSize) s.th += p.thSize; else s.thUnk++; } else s.noThumb++;
    if (hasT.has(p.id)) { s.devTn++; s.devT += szT.get(p.id) || 0; }
    if (p.thId && !hasT.has(p.id)) { s.getN++; if (p.thSize) s.get += p.thSize; else s.getUnk++; }
  }
  s.szT = szT;
  return s;
}
/* "≥" when some items have no recorded size yet */
const _stLabel = (bytes, unknown) => (unknown ? '\u2265 ' : '') + _upFmtBytes(bytes);
...
function openRetrieveModal() {
  ...
  if (!m) return;
  clearTimeout(m._closeT); m.classList.remove('closing');
  _pmForceRestart(m);
  m.classList.add('open');
...
async function retrieveRefreshStats() {
  ...
  _connBanner(note, isPcloudEnabled(), "Pcloud isn't connected on this device \u2014 connect it from the sidebar to retrieve your library.", "Pcloud is connected \u2014 you're ready to retrieve your library.");
  try {
    const s = await _vcStorageStats();
    const cloudPhotos = photos.filter(p => p.storage === 'cloud');
    const libBytes = cloudPhotos.length ? new Blob([JSON.stringify({ photos: cloudPhotos, folders })]).size : 0;
    const lib = cloudPhotos.length ? 1 : 0;                          // the library index is one file in Pcloud
    const unk = s.thUnk ? '\u2265 ' : '';
    const tot = s.main + s.th + libBytes;
    _upSetUnit('rtStatCloudSize', unk + _upFmtBytes(tot));
    set('rtStatCloudN', (s.cloudN + s.thN + lib) + ' files stored');
    set('rtStatMain', _upFmtBytes(s.main));
    set('rtStatMainSub', s.cloudN + ' file' + (s.cloudN !== 1 ? 's' : '') + ' \u00B7 stay in Pcloud');
    set('rtStatThumbs', _upFmtBytes(s.th));
    set('rtStatThumbsSub', s.devTn + ' of ' + s.thN + ' on this phone');
    set('rtStatLib', _upFmtBytes(libBytes));
    set('rtStatLibSub', s.cloudN + ' file' + (s.cloudN !== 1 ? 's' : '') + ' \u00B7 ' + folders.length + ' folder' + (folders.length !== 1 ? 's' : ''));
    // stacked bar: each part's share of what Pcloud holds (tiny parts keep a visible sliver)
    [['rtBarMain', s.main], ['rtBarThumbs', s.th], ['rtBarLib', libBytes]].forEach(([id, v]) => {
      const e = _rtEl(id); if (e) e.style.flex = (tot > 0 && v > 0 ? Math.max(100 * v / tot, 0.8) : 0) + ' 1 0%';
    });
...
async function retrieveRun() {
  ...
  _vcSetThumbsCleared(false);      // an explicit Retrieve re-enables downloading thumbnails
  const runBtn = _rtEl('rtRunBtn'), stopBtn = _rtEl('rtStopBtn'), box = _rtEl('rtBox');
  const title = _rtEl('rtTitle'), status = _rtEl('rtStatus'), icon = _rtEl('rtIcon');
  const seg1 = _rtEl('rtSeg1'), seg2 = _rtEl('rtSeg2'), seg3 = _rtEl('rtSeg3');
  const segLabel1 = _rtEl('rtSegLabel1'), segLabel2 = _rtEl('rtSegLabel2'), segLabel3 = _rtEl('rtSegLabel3');
  const _rtFill = (el) => el ? el.querySelector('.rt-seg-fill') : null;
  const _rtSetSeg = (pct) => {
    if (pct == null) return;
    const p1 = Math.max(0, Math.min(100, pct / 8 * 100));
    const p2 = pct <= 8 ? 0 : Math.max(0, Math.min(100, (pct - 8) / 92 * 100));
    const p3 = pct >= 100 ? 100 : 0;
    const f1 = _rtFill(seg1), f2 = _rtFill(seg2), f3 = _rtFill(seg3);
    if (f1) f1.style.width = p1 + '%';
    if (f2) f2.style.width = p2 + '%';
    if (f3) f3.style.width = p3 + '%';
    if (segLabel1) segLabel1.style.color = p1 > 0 ? 'var(--gold)' : 'rgba(255,255,255,0.3)';
    if (segLabel2) segLabel2.style.color = p2 > 0 ? 'var(--gold)' : 'rgba(255,255,255,0.3)';
    if (segLabel3) segLabel3.style.color = p3 > 0 ? 'var(--gold)' : 'rgba(255,255,255,0.3)';
  };
  const _rtSetBtn = (pct) => { const f = _rtEl('rtBtnFill'); if (f && pct != null) f.style.width = Math.max(0, Math.min(100, pct)) + '%'; };
  const ui = (t, s, pct) => { if (title) title.textContent = t + (pct != null && pct < 100 ? ' \u00B7 ' + Math.round(pct) + '%' : ''); if (status) status.textContent = s || ''; _rtSetSeg(pct); _rtSetBtn(pct); };
  if (runBtn) { runBtn.disabled = true; const bt = _rtEl('rtBtnText'); if (bt) bt.textContent = 'Retrieving\u2026'; const bf = _rtEl('rtBtnFill'); if (bf) bf.style.width = '0%'; }
  if (stopBtn) { stopBtn.style.display = 'block'; stopBtn.disabled = false; stopBtn.textContent = 'Stop'; }
  if (box) box.style.display = 'block';
  if (icon) { icon.className = 'fas fa-rotate'; icon.style.animation = 'spin 1s linear infinite'; }
  ui('Reading library…', 'Fetching the file list from Pcloud', 2);

  await _acquireCloudDirection('retrieve', 4000);
  let nT = 0, failed = 0, merged = null;
  try {
    ...
    const cloud = photos.filter(p => p.storage === 'cloud');
    const { hasT } = await _vcLocalState();
    const jobs = [];
    for (const p of cloud) {
      if (p.thId && !hasT.has(p.id)) jobs.push({ p, kind: 't', fid: p.thId });
    }

    // 3 ─ download the missing thumbnails
    let done = 0, idx = 0;
    const worker = async () => {
      while (!_rtStop && idx < jobs.length) {
        ...
        try {
          ...
          await _vcMergeRec(j.p.id, { t: blob });
          nT++;
        } catch (e) { failed++; }
        done++;
        ui('Saving thumbnails…', done + ' of ' + jobs.length + (failed ? ' · ' + failed + ' failed' : ''), 8 + Math.round(92 * done / Math.max(1, jobs.length)));
      }
    };
    ...
    ui(stopped ? 'Stopped' : 'Retrieve complete',
       merged.total + ' files \u00B7 ' + nT + ' thumbnails saved' + (failed ? ' \u00B7 ' + failed + ' failed' : '') + '. Main files stay in Pcloud and open when tapped.',
       stopped ? null : 100);
    try { render(); } catch (e) {}
    if (!stopped && !failed) showToast('✅ Retrieved from Pcloud');
  } catch (e) {
    if (icon) { icon.className = 'fas fa-triangle-exclamation'; icon.style.animation = 'none'; }
    ui('Retrieve failed', (e && e.message) || 'Could not reach Pcloud.', null);
  } finally {
    _releaseCloudDirection('retrieve');
    _rtRunning = false; _rtStop = false;
    if (runBtn) { runBtn.disabled = false; const bt = _rtEl('rtBtnText'); if (bt) bt.textContent = 'Retrieve from Pcloud'; const bf = _rtEl('rtBtnFill'); if (bf) bf.style.width = '0%'; }
...
/* ════════════════════════════════════════════════════════════════════════
     SECTION 8 — UPLOAD PAGE  (the page the Upload button on the Pcloud page opens)
     The upload itself is still tgPushAll(); its progress box lives on this page.
     ════════════════════════════════════════════════════════════════════════ */
function _upFmtBytes(n) {
  if (!n) return '0 B';
...
/* size of the library index (ids, names, folders) */
function _vcLibBytes() {
  const c = photos.filter(p => p.storage === 'cloud');
  return c.length ? new Blob([JSON.stringify({ photos: c, folders })]).size : 0;
}
/* real account usage from pCloud (cached for a minute) */
let _upAcct = { at: 0, v: null };
async function _upAccountUsage() {
  if (Date.now() - _upAcct.at < 60000) return _upAcct.v;
  let v = null;
  try {
    const r = await fetch(`https://${PCLOUD_HOST}/userinfo?access_token=${encodeURIComponent(PCLOUD_TOKEN)}`);
    const d = await r.json();
    if (d && d.result === 0 && d.quota) v = { used: d.usedquota || 0, quota: d.quota };
  } catch (e) { v = null; }
  _upAcct = { at: Date.now(), v };
  return v;
}

/* Upload page helpers (combined redesign) */
function _upSetHero(txt) {
  const e = document.getElementById('upEstTotal'); if (!e) return;
  const m = String(txt).match(/^(\D*?)([\d.,]+)\s*([A-Za-z]+)$/);
  e.textContent = '';
  if (!m) { e.textContent = txt; return; }
  e.appendChild(document.createTextNode(m[1] + m[2] + ' '));
  const u = document.createElement('span'); u.textContent = m[3]; e.appendChild(u);
}
/* stage: 0 idle, 1 uploading, 2 saving library, 3 done */
function _upStage(n) {
  const w = (id, v) => { const e = document.getElementById(id); if (e) e.style.width = v; };
  const c = (id, on, done) => { const e = document.getElementById(id); if (!e) return; e.classList.toggle('on', !!on); e.classList.toggle('done', !!done); };
  w('upSeg1', n >= 1 ? '100%' : '0%');
  w('upSeg3', n >= 3 ? '100%' : n === 2 ? '50%' : '0%');
  c('upLbl1', n >= 1, n >= 1); c('upLbl2', n >= 1, n >= 2); c('upLbl3', n >= 2, n >= 3);
}

/* Upload page: "1.1 GB" -> big number + small unit (used by the Pcloud storage card) */
function _upSetUnit(id, txt) {
...
/* Upload page: small floating "What Upload does" window. Any tap anywhere closes it. */
function _upOpenInfo() {
  const pop = document.getElementById('upInfoPop'), btn = document.getElementById('upInfoBtn'); if (!pop || !btn) return;
  pop.classList.remove('below'); pop.classList.add('open');
  const body = document.querySelector('#uploadModal .fs-panel-body');
  const top = body ? body.getBoundingClientRect().top : 0;
  if (btn.getBoundingClientRect().top - top < pop.offsetHeight + 16) pop.classList.add('below'); // no room above -> open below
  btn.setAttribute('aria-expanded', 'true');
}
function _upCloseInfo() {
  const pop = document.getElementById('upInfoPop'), btn = document.getElementById('upInfoBtn');
  if (pop) pop.classList.remove('open');
  if (btn) btn.setAttribute('aria-expanded', 'false');
}
function _upToggleInfo(e) {
  if (e) e.stopPropagation();
  const pop = document.getElementById('upInfoPop');
  if (pop && pop.classList.contains('open')) _upCloseInfo(); else _upOpenInfo();
}
document.addEventListener('click', () => { const p = document.getElementById('upInfoPop'); if (p && p.classList.contains('open')) _upCloseInfo(); });
document.addEventListener('keydown', e => { if (e.key === 'Escape') _upCloseInfo(); });

/* Upload + Retrieve pages: the top banner is red when Pcloud is not connected and turns green once it is. */
function _connBanner(note, connected, offMsg, onMsg) {
  if (!note) return;
  note.style.display = '';   // shown in both states (CSS decides the layout)
  const box = note.matches('.up-warn,.rt-warn') ? note : note.querySelector('.up-warn,.rt-warn');
  if (!box) return;
  box.classList.toggle('is-ok', !!connected);
  const t = box.querySelector('.conn-msg'); if (t) t.textContent = connected ? onMsg : offMsg;
}
/* Retrieve page: same small floating window as Upload. Any tap anywhere closes it. */
function _rtOpenInfo() {
  const pop = document.getElementById('rtInfoPop'), btn = document.getElementById('rtInfoBtn'); if (!pop || !btn) return;
  pop.classList.remove('below'); pop.classList.add('open');
  const body = document.querySelector('#retrieveModal .fs-panel-body');
  const top = body ? body.getBoundingClientRect().top : 0;
  if (btn.getBoundingClientRect().top - top < pop.offsetHeight + 16) pop.classList.add('below');
  btn.setAttribute('aria-expanded', 'true');
}
function _rtCloseInfo() {
  const pop = document.getElementById('rtInfoPop'), btn = document.getElementById('rtInfoBtn');
  if (pop) pop.classList.remove('open');
  if (btn) btn.setAttribute('aria-expanded', 'false');
}
function _rtToggleInfo(e) {
  if (e) e.stopPropagation();
  const pop = document.getElementById('rtInfoPop');
  if (pop && pop.classList.contains('open')) _rtCloseInfo(); else _rtOpenInfo();
}
document.addEventListener('click', () => { const p = document.getElementById('rtInfoPop'); if (p && p.classList.contains('open')) _rtCloseInfo(); });
document.addEventListener('keydown', e => { if (e.key === 'Escape') _rtCloseInfo(); });

async function uploadRefreshStats() {
  const set = (id, v) => { const e = document.getElementById(id); if (e) e.textContent = v; };
  const note = document.getElementById('upNote');
  const connected = isPcloudEnabled();
  _connBanner(note, connected, "Pcloud isn't connected on this device \u2014 connect it from the sidebar to upload.", "Pcloud is connected \u2014 you're ready to upload.");
  try {
    const s = await _vcStorageStats();
    const F = _upFmtBytes;
    const cloudLib = _vcLibBytes();

    // ── waiting files, and what they will add to Pcloud (main + thumbnail + library) ──
    const waiting = photos.filter(_isWaitingForUpload);
    const avgT = s.thN - s.thUnk > 0 ? s.th / (s.thN - s.thUnk) : 30 * 1024;
    const avgL = s.cloudN ? cloudLib / s.cloudN : 350;
    let estT = 0, guessed = false;
    for (const p of waiting) {
      if (s.szT.has(p.id)) estT += s.szT.get(p.id); else { estT += avgT; guessed = true; }
    }
    const estL = waiting.length * avgL;
    const ap = guessed ? '\u2248 ' : '';
    set('upStatWaiting', waiting.length + ' file' + (waiting.length !== 1 ? 's' : '') + ' waiting on this device');
    set('upEstMain', F(s.waiting));
    set('upEstThumbs', ap + F(estT));
    set('upEstLib', '\u2248 ' + F(estL));
    _upSetHero('\u2248 ' + F(s.waiting + estT + estL));
    { const tot = s.waiting + estT + estL, bw = (id, v) => { const e = document.getElementById(id); if (e) e.style.width = (tot > 0 ? Math.round(100 * v / tot) : 0) + '%'; };
      bw('upBarMain', s.waiting); bw('upBarThumbs', estT); bw('upBarLib', estL); }

    // ── what is in Pcloud now ──
    const unk = s.thUnk ? '\u2265 ' : '';
    const libFiles = s.cloudN ? 1 : 0;
    set('upStatVaultN', (s.cloudN + s.thN + libFiles).toLocaleString('en-US'));
    _upSetUnit('upStatVaultSize', unk + F(s.main + s.th + cloudLib));

    // ── the real Pcloud account usage (needs a network call) ──
    if (!connected) { set('upStatAccount', 'Not connected'); return; }
    set('upStatAccount', '\u2026');
    const acct = await _upAccountUsage();
    set('upStatAccount', acct ? F(acct.used) + ' of ' + F(acct.quota) + ' (' + Math.round(100 * acct.used / acct.quota) + '%)' : 'Unavailable');
  } catch (e) { /* stats are cosmetic */ }
}


/* ════════════════════════════════════════════════════════════════════════
     SECTION 9 — STORAGE PAGES  (opened from the two storage cards at the top of the sidebar)
        Device card → Device storage page  (what THIS PHONE holds)
        Cloud card  → Cloud storage page   (what is stored in Pcloud)
     ════════════════════════════════════════════════════════════════════════ */
function openDeviceStoragePage() {
  const m = document.getElementById('deviceStoragePage');
  if (!m) return;
  clearTimeout(m._closeT); m.classList.remove('closing');
  _pmForceRestart(m);
  m.classList.add('open');
  storageRefreshDevice();
  const body = m.querySelector('.fs-panel-body');
  if (body) body.scrollTop = 0;
}
function closeDeviceStoragePage() {
  const m = document.getElementById('deviceStoragePage');
  if (!m || !m.classList.contains('open') || m.classList.contains('closing')) return;
  _pmForceRestart(m);
  m.classList.add('closing');
...
function openCloudStoragePage() {
  const m = document.getElementById('cloudStoragePage');
  if (!m) return;
  clearTimeout(m._closeT); m.classList.remove('closing');
  _pmForceRestart(m);
  m.classList.add('open');
...
function closeCloudStoragePage() {
  const m = document.getElementById('cloudStoragePage');
  if (!m || !m.classList.contains('open') || m.classList.contains('closing')) return;
  _pmForceRestart(m);
  m.classList.add('closing');
...
/* the two numbers everything else is built from */
function _vcTotals(s) {
  const lib = _vcLibBytes();
  return {
    lib,
    device: lib + s.devT + s.waiting + s.cache,               // held on this phone
    cloud: s.main + s.th + (s.cloudN ? lib : 0),              // held in Pcloud
    cloudApprox: !!s.thUnk,
  };
}

/* Device storage page — only what this phone holds. No network calls. */
async function storageRefreshDevice() {
  const set = (id, v) => { const e = document.getElementById(id); if (e) e.textContent = v; };
  const F = _upFmtBytes;
  try {
    const s = await _vcStorageStats();
    const t = _vcTotals(s);
    set('stLocLib', F(t.lib));
    set('stLocThumbs', s.devTn + ' \u00B7 ' + F(s.devT));
    set('stLocWaiting', s.waitingN + ' \u00B7 ' + F(s.waiting));
    set('stLocCache', F(s.cache));
    set('stLocTotal', F(t.device));
  } catch (e) { /* stats are cosmetic */ }
}

/* Cloud storage page — only what is stored in Pcloud. */
async function storageRefreshCloud() {
  const set = (id, v) => { const e = document.getElementById(id); if (e) e.textContent = v; };
  const F = _upFmtBytes;
  try {
    const s = await _vcStorageStats();
    const t = _vcTotals(s);
    set('stCloudMain', s.cloudN + ' \u00B7 ' + F(s.main));
    set('stCloudThumbs', s.thN + ' \u00B7 ' + _stLabel(s.th, s.thUnk));
    set('stCloudLib', s.cloudN + ' files, ' + folders.length + ' folders \u00B7 ' + F(t.lib));
    set('stCloudTotal', (t.cloudApprox ? '\u2265 ' : '') + F(t.cloud));

    // main files in the cloud, by type
    const types = { image: [0, 0], video: [0, 0], gif: [0, 0] };
    for (const p of photos.filter(p => p.storage === 'cloud')) {
      const mt = _getMediaType(p); const k = mt === 'video' ? 'video' : mt === 'gif' ? 'gif' : 'image';
      types[k][0]++; types[k][1] += p.size || 0;
    }
    const names = { image: 'Images', video: 'Videos', gif: 'GIFs' };
    const box = document.getElementById('stTypes'), wrap = document.getElementById('stTypesWrap');
    if (box && wrap) {
      const rows = Object.keys(types).filter(k => types[k][0] > 0).map(k =>
        '<div style="display:flex;justify-content:space-between;gap:12px;padding:9px 0;font-size:13px;font-weight:700;color:rgba(255,255,255,0.6);"><span>' + names[k] +
        '</span><span style="color:white;text-align:right;">' + types[k][0] + ' \u00B7 ' + F(types[k][1]) + '</span></div>');
      box.innerHTML = rows.join('<div style="height:1px;background:rgba(255,255,255,0.06);"></div>');
      wrap.style.display = rows.length ? 'block' : 'none';
    }

    // real account usage (needs a network call)
    if (!isPcloudEnabled()) { set('stCloudAccount', 'Not connected'); return; }
    set('stCloudAccount', '\u2026');
    const acct = await _upAccountUsage();
    set('stCloudAccount', acct ? F(acct.used) + ' of ' + F(acct.quota) + ' (' + Math.round(100 * acct.used / acct.quota) + '%)' : 'Unavailable');
  } catch (e) { /* stats are cosmetic */ }
}


/* ════════════════════════════════════════════════════════════════════════
     Sidebar storage card: two separate numbers
        On device = library + thumbnails + files waiting + caches
        In cloud  = main files + thumbnails + library (all in Pcloud)
     Each card opens its own page (Device storage / Cloud storage) with the full detail.
     ════════════════════════════════════════════════════════════════════════ */
let _vcExtraTimer = null;
function _vcScheduleExtra() { clearTimeout(_vcExtraTimer); _vcExtraTimer = setTimeout(_vcRecomputeExtra, 1500); }
async function _vcRecomputeExtra() {
  _vcExtraTimer = null;
  try {
    const t = _vcTotals(await _vcStorageStats());
    const d = document.getElementById('dcDeviceValue'), c = document.getElementById('dcCloudValue');
    if (d) d.textContent = _upFmtBytes(t.device);
    if (c) c.textContent = (t.cloudApprox ? '\u2265 ' : '') + _upFmtBytes(t.cloud);
  } catch (e) { /* card keeps its last value */ }
}


/* One-time cleanup: an earlier build also kept a "preview" copy of every photo.
   Previews are gone, so free the space they used. */
async function _vcDropLegacyPreviews() {
  try {
    if (localStorage.getItem('pv_vc_prev_dropped') === '1') return;
    const db = await _vcDb();
    await new Promise((res, rej) => {
      const tx = db.transaction('thumb', 'readwrite');
      const rq = tx.objectStore('thumb').openCursor();
      rq.onsuccess = () => {
        const c = rq.result;
        if (!c) return;
        if (c.value && c.value.p) { const v = Object.assign({}, c.value); delete v.p; c.update(v); }
        c.continue();
      };
      tx.oncomplete = () => res();
      tx.onerror = tx.onabort = () => rej(tx.error);
    });
    localStorage.setItem('pv_vc_prev_dropped', '1');
  } catch (e) { /* try again next launch */ }
}


/* ════════════════════════════════════════════════════════════════════════
     SECTION 10 — CLEAR BUTTONS ON THE DEVICE STORAGE PAGE (this device only, never the cloud)
        Clear caches      → deletes the saved copies of files you opened
        Clear thumbnails  → deletes the thumbnails saved on this device
        Clear library     → empties the file list on this device (files waiting to upload
                            and folders are kept). Stays empty until Retrieve.
     ════════════════════════════════════════════════════════════════════════ */
function _vcLibCleared() { try { return localStorage.getItem('pv_lib_cleared') === '1'; } catch (e) { return false; } }
function _vcSetLibCleared(on) { try { if (on) localStorage.setItem('pv_lib_cleared', '1'); else localStorage.removeItem('pv_lib_cleared'); } catch (e) {} }
function _vcThumbsCleared() { try { return localStorage.getItem('pv_thumbs_cleared') === '1'; } catch (e) { return false; } }
function _vcSetThumbsCleared(on) { try { if (on) localStorage.setItem('pv_thumbs_cleared', '1'); else localStorage.removeItem('pv_thumbs_cleared'); } catch (e) {} }

/* re-read the cloud library before writing one (throws when it can't, which aborts the write) */
async function _vcHealLibrary() {
  const m = await _tgLoadManifest();
  if (!m || !Array.isArray(m.photos)) throw new Error('Library was cleared on this device \u2014 tap Retrieve first.');
  _vcMergeManifest(m);            // also switches the "cleared" flag off
}

function _stAfterClear() {
  _vcScheduleExtra();
  storageRefreshDevice();
  try { _updateStoragePill(); } catch (e) {}
}

function storageClearCaches() {
  showConfirmModal('Clear caches?',
    'Deletes the saved copies of files you opened on this device. Nothing in Pcloud is touched. Files download again when you open them.',
    'Clear caches', '#ff4444', async () => {
      try {
        await _vcTx('orig', 'readwrite', st => st.clear());
        await _vcTx('origmeta', 'readwrite', st => st.clear());
        showToast('\u{1F5D1}\uFE0F Caches cleared');
      } catch (e) { showToast('\u274C Could not clear caches'); }
      _stAfterClear();
    });
}

function storageClearThumbnails() {
  showConfirmModal('Clear thumbnails?',
    'Deletes the thumbnails saved on this device. Nothing in Pcloud is touched. They download again when a photo is shown, or all at once with Retrieve.',
    'Clear thumbnails', '#ff4444', async () => {
      try {
        await _vcTx('thumb', 'readwrite', st => st.clear());
        const old = Array.from(_vcMaps.t.values());
        _vcMaps.t.clear();
        setTimeout(() => old.forEach(u => { try { URL.revokeObjectURL(u); } catch (e) {} }), 120000);   // pictures on screen keep working
        _vcSetThumbsCleared(true);    // no background re-download until Retrieve
        showToast('\u{1F5D1}\uFE0F Thumbnails cleared');
      } catch (e) { showToast('\u274C Could not clear thumbnails'); }
      _stAfterClear();
    });
}

function storageClearLibrary() {
  showConfirmModal('Clear library?',
    'Empties the list of files (and their ids) on this device. Files waiting to upload and your folders are kept. Nothing in Pcloud is deleted \u2014 tap Retrieve to bring the list back.',
    'Clear library', '#ff4444', async () => {
      if (_cloudUploadBusy || _tgPushBusy) { showToast('\u23F3 Wait for the upload to finish'); return; }
      const had = photos.filter(p => p.storage === 'cloud').length;
      photos = photos.filter(p => p.storage !== 'cloud');      // never touches files that are still only on this phone
      try { await _vcDbDel('index', 'photos'); } catch (e) {}
      window._vcIndexSavedAt = 0;
      if (had > 0) _vcSetLibCleared(true);                      // blocks auto-refill and any cloud write until Retrieve
      try { render(); } catch (e) {}
      try { renderFolders(); } catch (e) {}
      try { updateCloudUI(); } catch (e) {}
      showToast(had ? '\u{1F5D1}\uFE0F Library cleared on this device' : 'Library was already empty');
      _stAfterClear();
    });
}
