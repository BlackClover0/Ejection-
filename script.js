...
async function uploadRefreshStats() {
  ...
  try {
    ...
    const locTotal = cloudLib + s.devT + s.devP + s.waiting + s.cache;
    set('upLocLib', F(cloudLib));
    set('upLocThumbs', s.devTn + ' \u00B7 ' + F(s.devT));
    set('upLocPrev', s.devPn + ' \u00B7 ' + F(s.devP));
    set('upLocWaiting', F(s.waiting));
    set('upLocCache', F(s.cache));
    set('upLocTotal', F(locTotal));
