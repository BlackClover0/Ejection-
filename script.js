...
async function retrieveRefreshStats() {
  ...
  try {
    ...
    set('rtStatFiles', String(s.cloudN));
    set('rtStatThumbs', s.devTn + ' / ' + s.thN + ' \u00B7 ' + _upFmtBytes(s.devT));
    set('rtStatPrev', s.devPn + ' / ' + s.pvN + ' \u00B7 ' + _upFmtBytes(s.devP));
    set('rtStatCache', _upFmtBytes(s.cache));
    set('rtStatTotal', _upFmtBytes(s.devT + s.devP + s.cache));
    set('rtStatMain', _upFmtBytes(s.main));
    set('rtStatToGet', s.getN ? s.getN + ' item' + (s.getN !== 1 ? 's' : '') + ' \u00B7 ' + (s.getUnk ? '\u2248 ' : '') + _upFmtBytes(s.get) : 'Up to date');
    const mEl = _rtEl('rtStatMissing');
    if (mEl) {
      mEl.style.display = s.noThumb ? 'block' : 'none';
      mEl.textContent = s.noThumb + ' file' + (s.noThumb !== 1 ? 's have' : ' has') + ' no thumbnail in Pcloud yet \u2014 created and uploaded automatically the first time they are shown.';
    }
...
async function uploadRefreshStats() {
  ...
  try {
    const s = await _vcStorageStats();
    set('upStatWaiting', s.waitingN + ' \u00B7 ' + _upFmtBytes(s.waiting));
    set('upStatCloud', s.cloudN + ' \u00B7 ' + _upFmtBytes(s.main));
    set('upStatThumbs', s.thN + ' / ' + s.cloudN + ' \u00B7 ' + _stLabel(s.th, s.thUnk));
    set('upStatPrev', s.pvN + ' / ' + s.cloudN + ' \u00B7 ' + _stLabel(s.pv, s.pvUnk));
    set('upStatTotal', (s.thUnk || s.pvUnk ? '\u2265 ' : '') + _upFmtBytes(s.main + s.th + s.pv));
