...
async function retrieveRefreshStats() {
  ...
  if (note) note.style.display = isPcloudEnabled() ? 'none' : 'block';
  try {
...
async function uploadRefreshStats() {
  ...
  try {
    ...
    const ap = guessed ? '\u2248 ' : '';
    set('upStatWaiting', waiting.length + ' file' + (waiting.length !== 1 ? 's' : '') + ' \u00B7 ' + F(s.waiting));
    set('upEstMain', F(s.waiting));
    set('upEstThumbs', ap + F(estT));
    set('upEstLib', '\u2248 ' + F(estL));
    set('upEstTotal', '\u2248 ' + F(s.waiting + estT + estL));
